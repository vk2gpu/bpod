...
void MCP23017::begin(BpodMenu &menu) {
    ...
    {
        menu.add("GPIOA" + std::to_string(i) + " " + std::string(gpio_a_pin(i) ? "ON" : "OFF"), [&menu, i, this](){ 
            set_gpio_a_pin(i, !gpio_a_pin(i));
            menu.set(i, "GPIOA" + std::to_string(i) + " " + std::string(gpio_a_pin(i) ? "ON" : "OFF"));
        });
    }
    for ( size_t i = 0; i < 8; i++ )
    {
        menu.add("GPIOB" + std::to_string(i) + " " + std::string(gpio_b_pin(i) ? "ON" : "OFF"), [&menu, i, this](){ 
            set_gpio_b_pin(i, !gpio_b_pin(i));
            menu.set(i + 8, "GPIOB" + std::to_string(i) + " " + std::string(gpio_b_pin(i) ? "ON" : "OFF"));
        });
    }
}
